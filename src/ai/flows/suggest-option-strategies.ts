...
const OptionStrategySchema = z.object({
    ...
    rationale: z.string().describe('A brief explanation of why this strategy is suitable for the given signal, including context for strike price and time frame selection (e.g., "slightly out-of-the-money at ~$155" and "30-60 days to expiration").')
});
...
const AggressiveOptionStrategySchema = z.object({
    ...
    rationale: z.string().describe('A brief explanation of why this strategy is aggressive and what scenario it would profit from, including a specific example strike price (e.g. "at ~$170") and very short-term expiration (e.g., "weekly" or "less than 2 weeks").')
});
...
3.  In the 'rationale' for standard strategies, you **MUST** provide a specific example strike price (e.g., "at ~$155") by calculating an appropriate level based on the latest close price. You must also include the qualitative description (e.g., "slightly out-of-the-money").
4.  In the 'rationale' for standard strategies, you **MUST** also suggest a suitable time frame for the option's expiration (e.g., "with 30-60 days to expiration").
...
4.  You **MUST** suggest a very short-term expiration (e.g., "weekly expiration," "in the next 1-2 weeks") and a specific example strike price (e.g., "at ~$170").
