...
export function MonteCarloSimulation({ marketData, currency, onSimulationComplete }: MonteCarloSimulationProps) {
  ...
  useEffect(() => {
    ...
    if (marketData && marketData.length > 30) { // Need sufficient data
      ...
      // We need to run this in a timeout to avoid blocking the main thread for too long
      // during initial render, allowing the UI to update with the loading state.
      setTimeout(() => {
        try {
          // The simulation expects chronological data (oldest to newest)
      ...
      }, 50); // Small delay to allow render thread to catch up
    } else if (marketData) {
        setLoading(false);
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 font-headline text-2xl">
          ...
          <span>30-Day Monte Carlo Forecast</span>
        </CardTitle>
                ...
                <Tooltip>
                    ...
                    <TooltipContent>
                        <div className='max-w-xs space-y-2'>
                          <p>This model simulates thousands of future price paths based on the stock's historical trend (drift) and volatility, providing a probable price range rather than a single target.</p>
                          <p className='font-semibold'>This forecast is based on recent historical data, making it adaptive to current market conditions but also more sensitive to short-term trends and noise.</p>
                        </div>
                    </TooltipContent>
                </Tooltip>
            </TooltipProvider>
      ...
      <CardContent>
        ...
        {result && !loading && (
          <div className="flex flex-col sm:flex-row justify-around items-center gap-6 p-4 rounded-lg bg-muted/50 text-center">
            ...
            <div className="flex flex-col items-center gap-1">
                 <h3 className="font-semibold text-sm text-muted-foreground">Average Simulated Target</h3>
                 <p className="font-bold text-lg text-foreground">
